...
//    }
    ...
    public void enrichmentTest() throws IOException, JSONException, DDIException {
        ...
        datasetTobeEnriched1.addAttribute(Field.DESCRIPTION.getName(), abstraction);
            ...
            if (file.isFile()) {
                if (file.getName().toLowerCase().endsWith("xml")) {
                    ...
                    for (int i = 0; i < reader.getEntryIds().size(); i++) {
                        ...
                        datasetTobeEnriched1.addAttribute(Field.NAME.getName(), title);
                        datasetTobeEnriched1.addAttribute(Field.DESCRIPTION.getName(), description);
                        datasetTobeEnriched1.addAttribute(Field.SAMPLE.getName(),sampleProtocol);
                        datasetTobeEnriched1.addAttribute(Field.DATA.getName(),dataProtocol);
