...
public class CrossReferencesProteinDatabasesService {
    ...
    public static Entry annotateGPMDBProteins(Entry dataset){
        if(dataset != null && dataset.getAdditionalFields() != null && !dataset.getAdditionalFields().isEmpty()){
            ...
            dataset.getAdditionalFields().getField().stream().filter(field -> field != null && field.getName().equalsIgnoreCase(uk.ac.ebi.ddi.xml.validator.utils.Field.GPMDB_MODEL.getName())).forEach(field -> {
    ...
    public static Dataset annotatePXCrossReferences(DDIDatasetAnnotationService datasetAnnotationService, Dataset dataset) throws RestClientException{
        ...
        if(dataset != null && dataset.getCrossReferences() != null){
            ...
            for(String crossRef: dataset.getCrossReferences().keySet()){
                if(crossRef.equalsIgnoreCase("PRIDE")){
                ...
                }else if(crossRef.equalsIgnoreCase("ProteomeExchange") || crossRef.equalsIgnoreCase("ProteomeXChange")){
                    ...
                    Map<String, Set<String>> keyValues = allDatasets.stream()
                            .collect(Collectors.groupingBy(Dataset::getDatabase, Collectors.mapping(Dataset::getAccession, Collectors.toSet())));
                    keyValues.entrySet().stream().forEach( x-> cross.put(x.getKey(), x.getValue()));
                } else{
